...
This example sends data without ack to the recieving unit (recive_no_ack).
...
//IMPORTANT:myTransferPortBaudRate must be same on recive_no_ack
...
void setup() {
  ...
  #if compileFor328P
    myTransferPort.begin(myTransferPortBaudRate);//Must be same baud rate on recive_no_ack.
  #elif compileForEsp8266
    myTransferPort.begin(myTransferPortBaudRate);//Must be same baud rate on recive_no_ack.
  #elif compileForEsp32
    myTransferPort.begin(myTransferPortBaudRate);//Must be same baud rate on recive_no_ack.
  #elif compileForEsp32Cam
    myTransferPort.begin(myTransferPortBaudRate, SERIAL_8N1, myTransferPortSerialRxPin, myTransferPortSerialTxPin);//Must be same baud rate on recive_no_ack.
  #endif
...
void loop() {
  ...
  if (millis() - previousMillisSentDataTypes >= intervalSendDataTypes) {//Check if it is time to transmit data.
    ...
    if (myTransfer.sendData(details(testByte), 1)){//Send var testByte as identifier 1.
      Serial.println(F("testByte sent."));Serial.println();
    } else {
    ...
    if (myTransfer.sendData(details(testFloat), 2)){//Send var testFloat as identifier 2.
       Serial.println(F("testFloat sent."));Serial.println();
    } else {
      if (myTransfer.getTxStatus() == myTransfer.TX_STATUS_DATA_TO_BIG){
        Serial.println();
        Serial.print(F("ERROR:Data packet exceeded the max size of "));Serial.print(myTransfer.txMaxSize);Serial.println(F(" bytes! Packet not sent!"));
    ...
    if (myTransfer.sendData(details(testUint8_tArray), 3)){//Send array testUint8_tArray as identifier 3.
      Serial.println(F("testUint8_tArray sent."));Serial.println();
    } else {
      if (myTransfer.getTxStatus() == myTransfer.TX_STATUS_DATA_TO_BIG){
        Serial.println();
        Serial.print(F("ERROR:Data packet exceeded the max size of "));Serial.print(myTransfer.txMaxSize);Serial.println(F(" bytes! Packet not sent!"));
    ...
    if (myTransfer.sendData(details(testCharArray), 4)){//Send array testCharArray as identifier 4.
      Serial.println(F("testCharArray sent."));Serial.println();
    } else {
      if (myTransfer.getTxStatus() == myTransfer.TX_STATUS_DATA_TO_BIG){
        Serial.println();
        Serial.print(F("ERROR:Data packet exceeded the max size of "));Serial.print(myTransfer.txMaxSize);Serial.println(F(" bytes! Packet not sent!"));
    ...
    if (myTransfer.sendData(details(txTestData), 5)){//Send struct txTestData as identifier 5.
    ...
    delay(delayTimeBetweenSendingDataTypes);
      ...
      Serial.println(F("Sending struct txTestData..."));
      if (myTransfer.sendData(details(txTestData), 0)){//Send struct txTestData as identifier 0.
        Serial.println(F("txTestData sent."));Serial.println();
      } else {
        if (myTransfer.getTxStatus() == myTransfer.TX_STATUS_DATA_TO_BIG){
          Serial.println();
      ...
      if (myTransfer.sendData(details(testCharArray2), 7)){//Send array testCharArray2 as identifier 7.
        Serial.println(F("testCharArray2 sent."));Serial.println();
      } else {
        if (myTransfer.getTxStatus() == myTransfer.TX_STATUS_DATA_TO_BIG){
          Serial.println();
          Serial.print(F("ERROR:Data packet exceeded the max size of "));Serial.print(myTransfer.txMaxSize);Serial.println(F(" bytes! Packet not sent!"));
          Serial.print(F("Size of testCharArray2 is "));Serial.print(sizeof(testCharArray2));Serial.println(F(" bytes. "));
          Serial.println();
        } else if (myTransfer.getTxStatus() == myTransfer.TX_INVALID_IDENTIFIER){
          Serial.println();
          Serial.println(F("ERROR:Invalid identifier, identifier 0 is reserved for ack! Packet not sent!"));
