...
This example recieves data without ack from the sending unit (send_no_ack).
...
//IMPORTANT:myTransferPortBaudRate must be same on send_no_ack
...
#define parseRecievedStructWithStruct true
  ...
  struct RECIEVED_TEST_DATA_STRUCTURE {//IMPORTANT:Must be same structure as in send_no_ack.
    bool myBool;//(1 byte)
    char myChar;//(1 byte)
...
void setup() {
  ...
  #if compileFor328P
    myTransferPort.begin(myTransferPortBaudRate);//Must be same baud rate on send_no_ack.
  #elif compileForEsp8266
    myTransferPort.begin(myTransferPortBaudRate);//Must be same baud rate on send_no_ack.
  #elif compileForEsp32
    myTransferPort.begin(myTransferPortBaudRate);//Must be same baud rate on send_no_ack.
  #elif compileForEsp32Cam
    myTransferPort.begin(myTransferPortBaudRate, SERIAL_8N1, myTransferPortSerialRxPin, myTransferPortSerialTxPin);//Must be same baud rate on send_no_ack.
  #endif
  ...
  if (myTransfer.receiveData()){//If we recieved anything valid.
    ...
    */
    ...
    if (myTransfer.getRxStatus() == myTransfer.RX_STATUS_BUFFER_TO_SMALL){//NOTE:This error might also show if the rx port buffer is not flushed quickly enough.
      Serial.print(F("ERROR:rx buffer size is set to small! ("));Serial.print(myTransfer.getRxBufferSize());Serial.println(F(" bytes.) Increase the rx buffer size."));
      Serial.println();
